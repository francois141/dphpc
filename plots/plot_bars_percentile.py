...
def plot_bars(args: argparse.Namespace, df: pd.DataFrame, dataset_name):
    ...
    ### Titles ###
    GPU_SPEC = f"NVIDIA {args.gpu.upper()}"
    plt.xlabel("Competitor - Matrix representation (K)", loc="center", fontdict={ "size": "medium" })
    plt.ylabel(f"p{int(args.percentile * 100)} runtime [ms]")
    plt.title(f"SDDMM {int(args.percentile * 100)}-percentile runtime on the {dataset_name} dataset ({df.iloc[0]['N']}x{df.iloc[0]['M']})\nRunning on {CPU_SPEC} & {GPU_SPEC}\n", loc="center", y=1.05, fontdict={ "weight": "bold", "size": "large" })
