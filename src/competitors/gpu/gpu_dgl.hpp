...
namespace Competitors {
    ...
    template <typename T>
    class GPUDgl: public SDDMM::Competitor<T> {
        public:

            GPUDgl()
            : SDDMM::Competitor<T>("GPU-dgl")
            {}
            ...
            virtual inline void init_csr(Dense<T> &A, Dense<T> &B, CSR<T> &S, CSR<T> &P) override {
                if (std::is_same<T, double>::value) {
                    ...
                    DEBUG_OUT("DGL SDDMM CSR kernel only supports floating point types. Skipping calculation.");
                    return;
                }
                ...
                auto csr = std::make_shared<dgl::sparse::CSR>();
                csr->num_rows = S.getRows();
                csr->num_cols = S.getCols();
                csr->indices = col_indices;
                csr->indptr = crow_indices;

                sparse_tensor = dgl::sparse::SparseMatrix::FromCSRPointer(csr, values, size);
            }
            ...
            virtual inline void run_csr(Dense<T> &A, Dense<T> &B, CSR<T> &S, CSR<T> &P) override {
                ...
                result = dgl::sparse::SDDMM(sparse_tensor, A_tensor, B_tensor);
            }
        ...
        private:
            ...
            c10::intrusive_ptr<dgl::sparse::SparseMatrix> sparse_tensor;

            c10::intrusive_ptr<dgl::sparse::SparseMatrix> result;
    };
