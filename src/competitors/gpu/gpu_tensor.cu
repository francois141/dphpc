...
__global__ void gpu_tensor_csr_kernel(float* A, float* B, float* S, float* P, int* cols, int* rows, int* start_idx, int M, int K, int N, int sparse_size, int row_size) {
...
void gpu_tensor_csr_wrapper(T* A_gpu, T* B_gpu, T* S_gpu, T* P_gpu, int* cols_gpu, int* rows_gpu, int* start_idx, int M, int K, int N, int sparse_size, int row_size) {
    ...
    gpu_tensor_csr_kernel<<<32, 32>>>(A_gpu, B_gpu, S_gpu, P_gpu, cols_gpu, rows_gpu, start_idx, M, K, N, sparse_size, row_size);
}
...
template void gpu_tensor_csr_wrapper<float>(float* A_gpu, float* B_gpu, float* S_gpu, float* P_gpu, int* cols_gpu, int* rows_gpu, int* start_idx, int M, int K, int N, int sparse_size, int row_size);
